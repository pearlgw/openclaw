...
describe("npm publication resume selection", () => {
  const workflowRef = `release-publish/${sourceSha.slice(0, 12)}-123`;
  function fixture(
    input: {
      runId?: string;
      tags?: string[];
    ...
    return {
      ...
      runGh: (args: string[]) => {
        const endpoint = args[1] ?? "";
        if (args[0] !== "api") {
          throw new Error("Resume observation must use GitHub REST.");
        }
        if (endpoint.includes("/workflows/openclaw-npm-release.yml/runs?")) {
          return JSON.stringify({ workflow_runs: [{ id: 122 }, { id: 123 }] });
        }
        if (endpoint.endsWith("/jobs?per_page=100")) {
          const id = endpoint.includes("/runs/122/") ? 122 : 123;
          const jobs = [
            { id: id + 100, name: "validate_publish_request", conclusion: "success" },
            { id: id + 200, name: "publish_openclaw_npm", conclusion: "success" },
          ].filter((job) => job.name !== input.missingJob);
          return JSON.stringify({ total_count: jobs.length, jobs });
        }
        if (endpoint.endsWith("/logs")) {
          const tags = input.tags ?? [
            endpoint.includes("/jobs/222/") ? "v2026.9.4" : `v${version}`,
          ];
          return tags.map((tag) => `2026-09-18T01:02:03Z   RELEASE_TAG: ${tag}`).join("\n");
        }
        if (endpoint.endsWith("/runs/123")) {
          return JSON.stringify({
            ...
            head_sha: input.headSha ?? sourceSha,
            html_url: "https://github.com/openclaw/openclaw/actions/runs/123",
            path: ".github/workflows/openclaw-npm-release.yml",
            workflow_id: 101,
          });
        }
        if (endpoint.endsWith("/workflows/openclaw-npm-release.yml")) {
          return JSON.stringify({ id: 101 });
        }
        if (endpoint.includes("/git/ref/tags/")) {
          return JSON.stringify({ object: { type: "commit", sha: sourceSha } });
        }
        throw new Error(`Unexpected resume endpoint: ${endpoint}`);
      },
    };
  }

  it("discovers the exact published release among successful runs on the tooling ref", () => {
    expect(resolveReleaseNpmResumeRun(fixture())).toBe("123");
  });

  it("accepts an explicitly supplied run with exact release and successful publication evidence", () => {
    expect(resolveReleaseNpmResumeRun(fixture({ runId: "123" }))).toBe("123");
  });

  it.each([
    { name: "another release", tags: ["v2026.9.4"] },
    { name: "missing tag evidence", tags: [] },
    { name: "conflicting tag evidence", tags: [`v${version}`, "v2026.9.4"] },
  ])("rejects an explicitly supplied run with $name", ({ tags }) => {
    expect(() => resolveReleaseNpmResumeRun(fixture({ runId: "123", tags }))).toThrow(
      "does not record the exact RELEASE_TAG",
    );
  });

  it.each(["validate_publish_request", "publish_openclaw_npm"])(
    "rejects a supplied run without successful %s",
    (missingJob) => {
      expect(() => resolveReleaseNpmResumeRun(fixture({ runId: "123", missingJob }))).toThrow(
        "lacks successful validate_publish_request and publish_openclaw_npm jobs",
      );
    },
  );

  it("still requires the existing protected workflow authority after exact-tag evidence matches", () => {
    expect(() =>
      resolveReleaseNpmResumeRun(fixture({ runId: "123", headSha: "b".repeat(40) })),
