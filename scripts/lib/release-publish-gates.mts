...
export function evaluatePluginNpmBootstrapGate(input: {
  packageName: string;
  ...
  const id = `plugin-npm.bootstrap.${input.packageName}`;
  if (input.packageVersion.includes("-beta.") && input.publishTag === "beta") {
    return {
      id,
      status: "PASS",
      message: `${input.packageName}@${input.packageVersion} is eligible for beta bootstrap.`,
      remediation: "",
    };
  }
  if (input.publishTag !== "latest") {
    return {
      id,
      status: "FAIL",
      message: `${input.packageName}@${input.packageVersion}: token bootstrap supports beta packages on beta or attested regular stable packages on latest; ${input.publishTag} is not eligible.`,
      remediation:
        "Complete a separately approved beta bootstrap and configure the package's trusted publisher before this channel's release.",
    };
  }
  const parent = evaluateReleaseBootstrapGate({ ...input, publishTag: input.npmDistTag });
  ...
  if (input.releaseTag !== `v${input.packageVersion}`) {
    return {
      id,
      status: "FAIL",
      message: `${input.packageName}@${input.packageVersion} is not covered by the ${input.releaseTag} bootstrap approval.`,
      remediation:
        "Use an approved stable release whose tag matches this package version, or establish its trusted publisher through the supported beta bootstrap flow.",
    };
  }
  return {
    id,
    status: "PASS",
    message: `${input.packageName}@${input.packageVersion} is eligible for the parent's stable bootstrap approval.`,
    remediation: "",
  };
}
