...
function readPublishSourceGit(args: string[]): string {
...
function readPublishSourceRecord(sourceSha: string, path: string): PublishPreflightRecord {
  return requirePreflightRecord(
    JSON.parse(readPublishSourceGit(["show", `${sourceSha}:${path}`])),
    path,
  );
}

export function verifyPublishAndroidPin({ sourceSha }: { sourceSha: string }): string {
  if (!/^[a-f0-9]{40}$/u.test(sourceSha)) {
    throw new Error("Android pin verification requires an exact release source SHA.");
  }
  const pin = readPublishSourceRecord(sourceSha, "apps/android/version.json").version;
...
function verifyPreflightCorePackages(
  ...
  const readPackage = (path: string) => readPublishSourceRecord(sourceSha, path);
