...
import { preparedDockerEvidenceFromFullRelease } from "./docker-release-artifacts.mjs";
...
export async function runReleasePublishPreflight(
  ...
  if (sourceSha) {
    const androidPin = await check(
      "release.android-pin",
      "Exact release source pins a valid Android version.",
      "Run the shared mobile cutter before cutting a replacement release tag.",
      () => verifyPublishAndroidPin({ sourceSha }),
    );
    if (androidPin && androidPin !== options.tag.slice(1).split("-", 1)[0]) {
      warn(
        "release.android-skip",
        `Android APK will be skipped: pin ${androidPin} differs from ${options.tag}.`,
        "Run scripts/mobile-release-version.ts --prepare before the next tag if an Android release is intended.",
      );
    }
  }
  ...
  if (evidenceRequired) {
    ...
    if (manifest) {
      ...
      if (!options.tag.includes("-alpha.")) {
        await check(
          "docker.prepared-descriptor",
          "Prepared Docker descriptor is valid or uses the supported historical preparation route.",
          "Use the exact FRV attempt with matching prepared Docker evidence; regenerate malformed evidence.",
          () =>
  ...
  if (registry) {
    ...
    if (registry.bootstrapCandidates.length > 0) {
      for (const candidate of registry.bootstrapCandidates) {
        rows.push(
          evaluatePluginNpmBootstrapGate({
  ...
  if (registry?.corePublished && options.publishOpenclawNpm !== false) {
    ...
    await check(
      ...
      () => {
        resume = resolveReleaseNpmResumeRun({
          repo: options.repo,
          tag: options.tag,
          workflowRef,
          runId: resume || undefined,
          runGh,
        });
        warn(
  ...
  if (
    ...
    if (sourceSha) {
      rows.push(
        ...(await inspectStableCloseoutPreflight({
          repo: options.repo,
          tag: options.tag,
          sourceSha,
          attempt,
